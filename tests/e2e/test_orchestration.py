...
from src.config import CITY, HOTEL_COMPETITORS
...
def test_main_orchestration(mocker):
    ...
    mock_create_webdriver = mocker.patch("main.create_webdriver", return_value=mock_driver_instance)
    ...
    # 5. Mock the Notification phase
    mock_send_email = mocker.patch("main.send_notification_email")
    ...
    # By default, DAYS_AHEAD=2 in test suites or config, therefore loops 2 times (date_list has n elements, range(len-1) is applied)
    assert mock_collect.call_count > 0
    assert mock_insert_prices.call_count > 0
    
    # Expect the email to proceed
